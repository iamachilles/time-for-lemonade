...
const FeatureCard = ({
  ...
  if (imageSrc) {
    return (
      <div
        className={`absolute ${position} z-30 max-w-[110px] md:block hidden animate-float`}
        style={{
          animationDuration: `${7 + Math.random() * 5}s`,
          transform: `translateX(${Math.random() * 20 - 10}px) translateY(${
            Math.random() * 30 - 15
          }px)`,
        }}
      >
  ...
  return (
    <div
      className={`absolute ${position} rounded-lg p-4 shadow-lg ${color} ${textColor} z-30 max-w-[110px] md:block hidden animate-float`}
      style={{
      ...
      <div className="container relative max-w-6xl mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-8 md:mb-16">
          <h2 className="text-2xl md:text-4xl font-bold mb-4">
            We write the boring stuff, you don't
          </h2>
          <p className="text-lg md:text-xl text-gray-600">
            Experience how Zest makes email seamless and enjoyable with powerful
            AI features
          </p>
        </div>
